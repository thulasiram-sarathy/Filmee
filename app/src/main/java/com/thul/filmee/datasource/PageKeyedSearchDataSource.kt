...
import com.thul.filmee.utils.parseSearchMoviesData
...
class PageKeyedSearchDataSource (
                                 val type:String,
...

{
    ...
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, MovieApiResponse>) {
        ...
        tmdbService.searchMovies(type,TMDB_KEY,1)
            .enqueue(object : retrofit2.Callback<ResponseBody> {
                ...
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful && response.code() == 200) {
                        ...
                        val moviesListingData = parseSearchMoviesData(moviesResponseString!!)
                        

                        Log.d("MOVIZ-A",moviesListingData.results.get(0).title)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, MovieApiResponse>) {
        afterParams = params
        networkState.postValue(NetworkState.LOADING)
        tmdbService.searchMovies(type, TMDB_KEY,params.key)
            .enqueue(object : retrofit2.Callback<ResponseBody> {
